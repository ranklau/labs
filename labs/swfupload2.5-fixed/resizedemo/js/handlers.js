...
var console = console || (function() {
	...
	window.attachEvent('onload',function() {
		...
		div.innerHTML = '<h3>debugger info</h3>';
	...
	return {
		...
		timeEnd: function(key) {
			console.log(key +": "+ (new Date().getTime() - map[key]));
		}
	}
})();
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesQueued > 0) {
			this.startResizedUpload(this.getFile(0).ID, RESIZE_WIDTH, RESIZE_HEIGHT, SWFUpload.RESIZE_ENCODING.JPEG, RESIZE_QUALITY);
		}
	} catch (ex) {
		this.debug(ex);
	}
...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		//console.log(bytesLoaded, file.size);
